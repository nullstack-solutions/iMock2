...
(function initTemplateManager(global) {
    ...
    const templateRepo = (() => {
        if (!global.TemplateRepoModule || typeof global.TemplateRepoModule.createTemplateRepo !== 'function') {
            Logger.error('TEMPLATES', 'TemplateRepoModule is not loaded');
            return null;
        }
        return global.TemplateRepoModule.createTemplateRepo({
            runtime: global,
    ...
    const templateActions = (() => {
        if (!global.TemplateActionsModule || typeof global.TemplateActionsModule.createTemplateActions !== 'function') {
            Logger.error('TEMPLATES', 'TemplateActionsModule is not loaded');
            return null;
        }
        return global.TemplateActionsModule.createTemplateActions({
            runtime: global,
            selectors: SELECTORS,
            getAllTemplates,
        });
    ...
    function normalizeUserTemplate(template) {
        return templateRepo?.normalizeUserTemplate
            ? templateRepo.normalizeUserTemplate(template)
            : ({ ...template });
    }

    function invalidateTemplateCache(scope = 'all') {
        return templateRepo?.invalidateTemplateCache
            ? templateRepo.invalidateTemplateCache(scope)
            : undefined;
    }

    function getBuiltInTemplates() {
        return templateRepo?.getBuiltInTemplates
            ? templateRepo.getBuiltInTemplates()
            : [];
    }

    function readUserTemplates() {
        return templateRepo?.readUserTemplates
            ? templateRepo.readUserTemplates()
            : [];
    }

    function persistUserTemplates(templates) {
        return templateRepo?.persistUserTemplates
            ? templateRepo.persistUserTemplates(templates)
            : undefined;
    }
    ...
    function findTemplateById(templateId) {
        return templateRepo?.findTemplateById
            ? templateRepo.findTemplateById(templateId)
            : null;
    }

    function getTemplatesWithMeta() {
        return templateRepo?.getTemplatesWithMeta
            ? templateRepo.getTemplatesWithMeta()
            : [];
    }

    function createOption(template) {
        return templateActions?.createOption
            ? templateActions.createOption(template)
            : document.createElement('option');
    }

    function populateSelectors() {
        if (templateActions?.populateSelectors) {
            templateActions.populateSelectors();
        }
    }

    function toJsonBody(bodyValue) {
        return templateRepo?.toJsonBody
            ? templateRepo.toJsonBody(bodyValue)
            : null;
    }
    ...
    function resolveTemplatePath(value, path) {
        return templateRepo?.resolveTemplatePath
            ? templateRepo.resolveTemplatePath(value, path)
            : undefined;
    }

    function formatFeatureValue(value) {
        return templateRepo?.formatFeatureValue
            ? templateRepo.formatFeatureValue(value)
            : '';
    }

    function getTemplateFeature(template) {
        return templateRepo?.getTemplateFeature
            ? templateRepo.getTemplateFeature(template)
            : null;
    }

    function getTemplateHeadline(template) {
        return templateRepo?.getTemplateHeadline
            ? templateRepo.getTemplateHeadline(template)
            : '';
    }

    function buildTemplatePreview(template) {
        return templateRepo?.buildTemplatePreview
            ? templateRepo.buildTemplatePreview(template)
            : '[unavailable template preview]';
    }

    function copyTextToClipboard(text) {
        if (templateActions?.copyTextToClipboard) {
            return templateActions.copyTextToClipboard(text);
        }
